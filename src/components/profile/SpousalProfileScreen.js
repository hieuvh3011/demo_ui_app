...
const SpousalProfileScreen = props => {
  ...
  const {genderList, photoUri, firstName, lastName} = useSelector(
    state => state?.user?.spousalProfile,
  );
  const dispatch = useDispatch();

  const clearFirstName = () => {
    dispatch(typeSpousalFirstName(''));
  };

  const onChangeFirstName = text => {
    dispatch(typeSpousalFirstName(text));
  };

  const onChangeLastName = text => {
    dispatch(typeSpousalLastName(text));
  };

  const clearLastName = () => {
    dispatch(typeSpousalLastName(''));
  };

  const _onPressGender = gender => {
    if (gender.isSelect === false) {
      dispatch(selectSpousalGender(gender));
    }
  };
  ...
  const _openGallery = async () => {
    ...
    launchImageLibrary(options, response => {
      ...
      if (!response.didCancel) {
        ...
        if (assets?.uri) {
          dispatch(selectSpousalPhoto(assets?.uri));
        }
  ...
  const _closePickerModal = () => setShowImagePicker(false);
